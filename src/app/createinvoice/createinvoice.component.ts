...
export class CreateinvoiceComponent implements OnInit {
  ...
  invoiceform = this.builder.group({
    ...
    remarks: this.builder.control('',[Validators.required,Validators.maxLength(255)]),
  ...
  SaveInvoice() {
    if (this.invoiceform.valid) {
      ...
      const requestBody = {
        ...
        "totalprice":this.invoiceform.get("netTotal")?.value,
  ...
  countrychange() {
    let countrycode = parseInt(this.invoiceform.get("countryId")?.value|| '0', 10) ?? 0;
    let Dis_Code = parseInt(this.invoiceform.get("customerId")?.value|| '0', 10) ?? 0;
    this.service.Getcountry().subscribe(res => {
      let countrydata: any;
      countrydata = res;
      if (countrydata != null) {
    ...
    this.service.GetPharmacyByCon(countrycode).subscribe(res => {
    
      this.masterpharmacy = res;
     
    });
    if( countrycode != 0 && Dis_Code != 0)
    {
      this.service.GetAddressFillter(Dis_Code,countrycode).subscribe(res => {
  ...
  ConChange() {
    let con_id = this.invoiceform.get("countryId")?.value;
    this.service.GetPharmacyByCon(con_id).subscribe(res => {
  ...
  productchange(index: any) {
    ...
    this.service.GetProductbycode(productcode).subscribe(res => {
      ...
      if (proddata != null) {
        ...
        this.invoiceproduct.get("salesPrice")?.setValue(proddata.data.price);
        this.invoiceproduct.get("productName")?.setValue(proddata.data.name);
